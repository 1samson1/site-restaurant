<?php 
    $modules = array(
        array(
            "name" => "settings",
            "verbose_name" => "Настройки системы",
            "image" => "skin/img/icon.png",
            "description" => "Управление настройками DuckEngine",
        ),
        array(
            "name" => "static",
            "verbose_name" => "Статические страницы",
            "image" => "skin/img/icon.png",
            "description" => "Создание и редактирование страниц, которые как правило редко изменяются и имеют постоянный адрес",
        ),
        array(
            "name" => "users",
            "verbose_name" => "Пользователи",
            "image" => "skin/img/icon.png",
            "description" => "Управление пользователями на сайте ,создание и редактирование их профилей",
        ),
        array(
            "name" => "comments",
            "verbose_name" => "Комментарии",
            "image" => "skin/img/icon.png",
            "description" => "Инструменты для быстрого и удобного модерирования комментариев",
        ),
        array(
            "name" => "news",
            "verbose_name" => "Новости",
            "image" => "skin/img/icon.png",
            "description" => "Создание и редактирование новостей на сайте",
        ),
        array(
            "name" => "categories",
            "verbose_name" => "Категории товаров",
            "image" => "skin/img/icon.png",
            "description" => "Управление категорияси товаров , создание, редактирование и удаление",
        ),
        array(
            "name" => "tovars",
            "verbose_name" => "Товары",
            "image" => "skin/img/icon.png",
            "description" => "Добавление, редактирование и удаление товаров представленных на сайте",
        ),
        array(
            "name" => "orders",
            "verbose_name" => "Заказы",
            "image" => "skin/img/icon.png",
            "description" => "Просмотр заказов пользователей и выполнение над ними действий",
        ),
    );
?>